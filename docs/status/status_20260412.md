...
## Propuesta: qué hacemos esta semana (entre 3)

### Lo que propongo discutir hoy

**Pregunta central:** De todo lo que falta, ¿qué podemos cerrar esta semana entre Víctor, Mariano e Ignacio para que la demo sea sólida?

### Opción A — Foco en navegación + UX (pragmático)

| Quién | Tarea | Por qué |
|-------|-------|---------|
| Víctor | Páginas `/song/:id`, `/album/:id`, `/artist/:addr` | Sin navegación interna la app se siente rota — links existen pero dan 404 |
...
**Session keys:** dejarlo para la semana siguiente o simplificar con `approve(MAX_UINT256)` una sola vez.

### Opción B — Foco en session keys + navegación (ambicioso)

| Quién | Tarea | Por qué |
|-------|-------|---------|
| Víctor | Páginas `/song/:id`, `/album/:id`, `/artist/:addr` | Mismo razonamiento |
| Mariano | Retomar Smart Accounts + session keys | El diseño ya existe en `AA_SESSION_KEYS_DESIGN.md`, los contratos están escritos |
...
| Quién | Tarea | Por qué |
|-------|-------|---------|
| Víctor | Páginas de navegación + doc final | Cubre dos items de la lista |
| Mariano | `approve(MAX_UINT256)` one-shot + filtros marketplace | El approve es un quick win que elimina el popup sin session keys reales |
