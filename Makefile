...
SONGS_PER_HOLDER ?= 5
MONTHS      ?= 6
DATABASE_URL ?= postgres://wave3:wave3@localhost:5432/wave3
DATABASE_SCHEMA ?= wave3
reset-local:
...
# Seed fake historical Ponder data for portfolio/marketplace demos
# Usage: make seed-historical [HOLDER=0x...] [SONGS_PER_HOLDER=5] [MONTHS=6] [DATABASE_URL=postgres://wave3:wave3@localhost:5432/wave3] [DATABASE_SCHEMA=wave3]
seed-historical:
	HOLDER=$(HOLDER) SONGS_PER_HOLDER=$(SONGS_PER_HOLDER) MONTHS=$(MONTHS) DATABASE_URL=$(DATABASE_URL) DATABASE_SCHEMA=$(DATABASE_SCHEMA) python seed/seed_historical.py

# Preview new releases albums (dry-run, no upload)
# Usage: make seed-new-releases-preview [SEED=12345678] [SAMPLE=6]
SEED_NR   ?= 12345678
SAMPLE_NR ?= 6
seed-new-releases-preview:
	python seed/seed_new_releases.py --dry-run --seed $(SEED_NR) --sample $(SAMPLE_NR)

# Upload new releases albums to chain
# Usage: make seed-new-releases DEPLOYER_PRIVATE_KEY=0x... [SEED=12345678] [SAMPLE=6]
seed-new-releases:
	NETWORK=baseSepolia DEPLOYER_PRIVATE_KEY=$(DEPLOYER_PRIVATE_KEY) RPC_URL=$(RPC_URL) python seed/seed_new_releases.py --seed $(SEED_NR) --sample $(SAMPLE_NR)

# Remove fake historical Ponder data generated by seed-historical
# Usage: make seed-historical-remove [DATABASE_URL=postgres://wave3:wave3@localhost:5432/wave3] [DATABASE_SCHEMA=wave3]
seed-historical-remove:
	DATABASE_URL=$(DATABASE_URL) DATABASE_SCHEMA=$(DATABASE_SCHEMA) python seed/seed_historical.py --remove
