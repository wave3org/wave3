...
contract Wave3SmartAccount is EIP712 {
    ...
    bytes32 private constant AUTHORIZE_SESSION_KEY_TYPEHASH = keccak256(
    ...
    bytes32 private constant EXECUTE_SESSION_TYPEHASH = keccak256(
    ...
    function revokeSessionKey(
        ...
        bytes32 structHash = keccak256(
            abi.encode(REVOKE_SESSION_KEY_TYPEHASH, sessionKey, nonce, deadline)
        );
        bytes32 digest = _hashTypedDataV4(structHash);
        address signer = ECDSA.recover(digest, signature);
        if (signer != owner) {
    ...
    function _verifyExecuteSessionSignature(
        ...
        bytes32 structHash = _buildExecuteSessionStructHash(
            sessionKey,
            target,
            value,
            data,
